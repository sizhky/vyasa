...
def render_post_detail(path, htmx, request, *, get_root_folder, effective_abbreviations, find_folder_note_file, slug_to_title, layout, get_blog_title, not_found, parse_frontmatter, resolve_markdown_title, from_md, logger, PathCls=Path):
    ...
    if not file_path or not file_path.exists():
        if folder_path and folder_path.exists() and folder_path.is_dir():
            ...
            if note_file:
                ...
                return RedirectResponse(content_url_for_slug(content_slug_for_path(note_file)), status_code=307)
    ...
    present_button = A(UkIcon("monitor", cls="w-4 h-4"), "Present", href=content_url_for_slug(path, prefix="/slides", suffix="/slide-1"), hx_boost="false", cls="vyasa-page-action-button inline-flex items-center gap-2 px-3 py-2 rounded-md text-sm")
...
def render_index(htmx, request, *, get_blog_title, find_index_file_fn, parse_frontmatter, resolve_markdown_title, get_root_folder, from_md, layout, not_found=None, logger=None):
    ...
    if index_file:
        ...
        present_button = A(UkIcon("monitor", cls="w-4 h-4"), "Present", href=content_url_for_slug(index_path, prefix="/slides", suffix="/slide-1"), hx_boost="false", cls="vyasa-page-action-button inline-flex items-center gap-2 px-3 py-2 rounded-md text-sm")
        page_content = Div(
            Div(
    ...
    present_button = A(UkIcon("monitor", cls="w-4 h-4"), "Present", href=content_url_for_slug(FALLBACK_HOME_SLUG, prefix="/slides", suffix="/slide-1"), hx_boost="false", cls="vyasa-page-action-button inline-flex items-center gap-2 px-3 py-2 rounded-md text-sm")
