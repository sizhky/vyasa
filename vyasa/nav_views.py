...
def navbar_panel_toggle(icon, title, onclick):
    return Button(UkIcon(icon, cls="w-5 h-5"), title=title, cls="p-2 hover:bg-slate-800 rounded transition-colors", type="button", onclick=onclick)
...
def navbar_view(blog_title, theme_toggle_node, show_mobile_menus=False, htmx_nav=True, posts_menu_items=None, compact_mode=False, updated_label=None, mobile_extra_controls=()):
    ...
    posts_toggle = navbar_panel_toggle("panel-left", "Toggle library", "window.__vyasaTogglePostsPanel && window.__vyasaTogglePostsPanel()") if show_mobile_menus and not compact_mode else None
    ...
    if show_mobile_menus:
        ...
        mobile = Div(navbar_panel_toggle("panel-left", "Toggle library", "window.__vyasaTogglePostsPanel && window.__vyasaTogglePostsPanel()"), mobile_title, Div(navbar_search_trigger(), theme_toggle_node, *mobile_extra_controls, cls="flex items-center gap-1"), cls="flex items-center justify-between xl:hidden")
