...
def navbar_view(blog_title, theme_toggle_node, show_mobile_menus=False, htmx_nav=True, posts_menu_items=None, compact_mode=False, updated_label=None, mobile_extra_controls=()):
    ...
    right = Div(navbar_search_trigger(compact=True), menu, theme_toggle_node, cls="flex items-center gap-3")
    if compact_mode:
    ...
    if show_mobile_menus:
        ...
        mobile = Div(Button(UkIcon("menu", cls="w-5 h-5"), title="Toggle file tree", id="mobile-posts-toggle", cls="p-2 hover:bg-slate-800 rounded transition-colors", type="button", onclick="window.__vyasaTogglePostsPanel && window.__vyasaTogglePostsPanel()"), mobile_title, Div(navbar_search_trigger(), *mobile_extra_controls, theme_toggle_node, cls="flex items-center gap-1"), cls="flex items-center justify-between xl:hidden")
