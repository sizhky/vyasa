...
def navbar_view(blog_title, theme_toggle_node, show_mobile_menus=False, htmx_nav=True, posts_menu_items=None, compact_mode=False):
    ...
    if compact_mode:
        ...
        return Div(row, cls="vyasa-navbar-card overflow-visible bg-slate-900 text-white p-4 my-4 rounded-lg shadow-md dark:bg-slate-800")
    if show_mobile_menus:
        mobile = Div(Button(UkIcon("menu", cls="w-5 h-5"), title="Toggle file tree", id="mobile-posts-toggle", cls="p-2 hover:bg-slate-800 rounded transition-colors", type="button"), A(blog_title, href="/", cls="flex-1 px-4 text-center truncate", **home_link_attrs), Div(Button(UkIcon("list", cls="w-5 h-5"), title="Toggle table of contents", id="mobile-toc-toggle", cls="p-2 hover:bg-slate-800 rounded transition-colors", type="button"), theme_toggle_node, cls="flex items-center gap-1"), cls="flex items-center justify-between xl:hidden")
        desktop = Div(left, right, cls="hidden xl:flex items-center justify-between")
        return Div(mobile, desktop, cls="vyasa-navbar-card overflow-visible bg-slate-900 text-white p-4 my-4 rounded-lg shadow-md dark:bg-slate-800")
    return Div(left, right, cls="vyasa-navbar-card overflow-visible flex items-center justify-between bg-slate-900 text-white p-4 my-4 rounded-lg shadow-md dark:bg-slate-800")
