...
def resolve_theme_extension(
    theme_id: str,
    registry: dict[str, ThemeExtension],
    *,
    ...
    if entry.randomizable and entry.choices:
        ...
        choice = picker.choice(list(entry.choices))
        return resolve_theme_extension(choice, registry, rng=rng, _seen=seen)
    if entry.choices:
        return resolve_theme_extension(entry.choices[0], registry, rng=rng, _seen=seen)
    return {}, ""


def theme_extension_payloads(
    registry: dict[str, ThemeExtension],
    *,
    rng: random.Random | None = None,
) -> dict[str, dict[str, str]]:
    payloads = {}
    for theme_id in registry:
        theme, _ = resolve_theme_extension(theme_id, registry, rng=rng)
        payloads[theme_id] = theme
    return payloads
...
def theme_extension_meta(registry: dict[str, ThemeExtension]) -> dict[str, dict]:
    return {
        theme_id: {
            ...
            "randomizable": entry.randomizable,
        }
        for theme_id, entry in registry.items()
    }
...
def list_theme_files() -> list[str]:
    return sorted(
        path.name[:-5]
        for path in resources.files("vyasa.themes").iterdir()
