---
name: skill-old
description: Archived previous Vyasa skill, kept for fallback comparison while the routed Vyasa skill is active.
version: "0.1.4"
---

# Vyasa

Use this skill for real Vyasa behavior and for building new Vyasa extensions. All reference files are self-contained.
...
5. If the user is asking for a new Vyasa capability, first decide whether it belongs in content syntax, layout/theme config, or a real extension.
6. For extension work, learn the contract from `vyasa/extensions.py` first, then copy the smallest existing built-in extension that matches the job.
...
13. For extension work, prefer the real seams first: route registration in `vyasa/extensions_builtin/*`, render hooks in extension modules, and asset bundles declared through `vyasa/extensions.py`; do not wire feature behavior back into `core.py` unless it is truly shell-global.
...
- New feature logic should usually be an extension, not a `core.py` edit.
- A Vyasa extension means: declare `ExtensionMeta`, implement `EXTENSION.register(app)`, register only the capabilities you declared, and keep feature code inside its own module/package.
- If an extension needs browser assets, keep them under `vyasa/extensions_builtin/<extension>/static/` and declare them as `AssetBundle`s instead of stuffing feature code into the global shell script.
- If an extension changes page behavior, make runtime and static build agree; emitted HTML and copied assets must match.
- Theme implementation lives in `vyasa/theme_extensions/`. `vyasa/extensions_builtin/themes/` is compatibility surface, not the primary place for new theme logic.
...
- Inside `items`/`tasks`, prefer normal markdown links in author text: attr values like `owner: [Alice](team/alice)` or `spec: [API](guide#api)`, and group/item labels like `Docs [API](guide#api):`, instead of a special `href:` attr.
