...
def test_extensions_default_preset_when_section_omitted(tmp_path, monkeypatch):
    ...
    assert plan.selected_by_category["route"] == ("slides", "auth_rbac", "sidebar_routes", "git_refs", "annotations", "feedback", "bookmarks", "api_catalog", "filesystem_routes")
...
def test_default_route_extensions_include_annotations():
    ...
    assert "/api/feedback" in prefixes
    assert "annotations" in runtime.storage_namespaces
    assert "feedback" in runtime.storage_namespaces
